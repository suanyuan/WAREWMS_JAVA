...
$(function() {
	...
	ezuiDatagrid = $('#ezuiDatagrid').datagrid({
		url : '<c:url value="/rptOrderPackingPerformController.do?showDatagrid"/>',
		method:'POST',
		toolbar : '#toolbar',
		title: '',
		pageSize : 50,
		...
		idField : 'id',
		columns : [[
			 {field: 'sdate',		title: '日期',	width: 100 },
	         {field: 'userId',		title: '工号',	width: 100 },
		     {field: 'userName',		title: '姓名',	width: 100 },
		     {field: 'customerid',		title: '客户编码',	width: 100 },
			 {field: 'qtyordered',		title: '计件 ',	width: 100 },
			 {field: 'ttim',		title: '计时',	width: 100 }
		]],
		...
		onRowContextMenu : function(event, rowIndex, rowData) {
			...
			ajaxBtn($('#menuId').val(), '<c:url value="/rptOrderPackingPerformController.do?getBtn"/>', ezuiMenu);
			$(this).datagrid('unselectAll');
		}
	});
...
var add = function(){
	processType = 'add';
	$('#rptOrderPackingPerformId').val(0);
...
var del = function(){
	...
	if(row){
		$.messager.confirm('<spring:message code="common.message.confirm"/>', '<spring:message code="common.message.confirm.delete"/>', function(confirm) {
			if(confirm){
				$.ajax({
					url : 'rptOrderPackingPerformController.do?delete',
					data : {id : row.id},
					type : 'POST',
					dataType : 'JSON',
...
var commit = function(){
	var url = '';
	if (processType == 'edit') {
		url = '<c:url value="/rptOrderPackingPerformController.do?edit"/>';
	}else{
		url = '<c:url value="/rptOrderPackingPerformController.do?add"/>';
	}
...
var doExport = function(){
	if(navigator.cookieEnabled){
		...
		//--导出Excel
		var formId = ajaxDownloadFile(sy.bp()+"/rptOrderPackingPerformController.do?exportOrderpackPerformDataToExcel", param);
...
<body>
	...
	<div id='ezuiDialog' style='padding: 10px;'>
		<form id='ezuiForm' method='post'>
			<input type='hidden' id='rptOrderPackingPerformId' name='rptOrderPackingPerformId'/>
			<table>
				<tr>
					<th>待输入0</th>
