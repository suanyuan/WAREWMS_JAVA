...
$(function() {
	...
	ezuiDatagrid = $('#ezuiDatagrid').datagrid({
		url : '<c:url value="/rptSortPerformanceController.do?showDatagrid"/>',
		method:'POST',
		toolbar : '#toolbar',
		title: '',
		pageSize : 50,
		...
		onRowContextMenu : function(event, rowIndex, rowData) {
			...
			ajaxBtn($('#menuId').val(), '<c:url value="/rptSortPerformanceController.do?getBtn"/>', ezuiMenu);
			$(this).datagrid('unselectAll');
		}
	});
...
var del = function(){
	...
	if(row){
		$.messager.confirm('<spring:message code="common.message.confirm"/>', '<spring:message code="common.message.confirm.delete"/>', function(confirm) {
			if(confirm){
				$.ajax({
					url : 'rptSortPerformanceController.do?delete',
					data : {id : row.id},
					type : 'POST',
					dataType : 'JSON',
...
var commit = function(){
	var url = '';
	if (processType == 'edit') {
		url = '<c:url value="/rptSortPerformanceController.do?edit"/>';
	}else{
		url = '<c:url value="/rptSortPerformanceController.do?add"/>';
	}
...
var doExport = function(){
	if(navigator.cookieEnabled){
		...
		//--导出Excel
		var formId = ajaxDownloadFile(sy.bp()+"/rptSortPerformanceController.do?exportSortPerformDataToExcel", param);
...
<body>
	...
	<div id='ezuiDialog' style='padding: 10px;'>
		<form id='ezuiForm' method='post'>
			<input type='hidden' id='rptSortPerformanceId' name='rptSortPerformanceId'/>
			<table>
				<tr>
					<th>待输入0</th>
