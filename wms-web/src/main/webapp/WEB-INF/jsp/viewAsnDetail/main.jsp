...
$(function() {
	...
	ezuiDatagrid = $('#ezuiDatagrid').datagrid({
		url : '<c:url value="/viewAsnDetailController.do?showDatagrid"/>',
		method:'POST',
		toolbar : '#toolbar',
		title: '待输入标题',
		pageSize : 50,
		...
		idField : 'id',
		columns : [[
		            {field: 'customerid',		title: '客户编号',	width: 34 },
		            {field: 'asnno',		title: 'ASN编号',	width: 44 },
		            {field: 'asnreference1',		title: '客户入库编号',	width: 50 },
		            {field: 'sku',		title: 'SKU',	width: 44 },
		            {field: 'descrC',		title: '商品名称',	width: 44 },
		            {field: 'reservedfield01',		title: '商品单位',	width: 34 },
		            {field: 'expectedqty',		title: '预期入库数量',	width: 50 },
		            {field: 'receivedqty',		title: '入库数量',	width: 34 },
		            {field: 'codenameC',		title: '入库类型',	width: 34 },
		            {field: 'codenameC1',		title: '入库状态',	width: 34 },
		            {field: 'cube',		title: '入库体积',	width: 34 },
		            {field: 'receivedtime',		title: '商品入库日期',	width: 50 },
		            {field: 'lastreceivingtime',		title: '最后入库日期',	width: 50 },
					{field: 'addtime',		title: '创建日期',	width: 44 },
					{field: 'edittime',		title: '关闭日期',	width: 44 },
					{field: 'userdefine2',		title: '自定义2',	width: 44 },
					{field: 'notes',		title: '备注',	width: 44 }	
		...
		onRowContextMenu : function(event, rowIndex, rowData) {
			...
			ajaxBtn($('#menuId').val(), '<c:url value="/viewAsnDetailController.do?getBtn"/>', ezuiMenu);
			$(this).datagrid('unselectAll');
		}
	});
...
var del = function(){
	...
	if(row){
		$.messager.confirm('<spring:message code="common.message.confirm"/>', '<spring:message code="common.message.confirm.delete"/>', function(confirm) {
			if(confirm){
				$.ajax({
					url : 'viewAsnDetailController.do?delete',
					data : {id : row.id},
					type : 'POST',
					dataType : 'JSON',
...
var commit = function(){
	var url = '';
	if (processType == 'edit') {
		url = '<c:url value="/viewAsnDetailController.do?edit"/>';
	}else{
		url = '<c:url value="/viewAsnDetailController.do?add"/>';
	}
...
<body>
				...
				<fieldset>
					...
					<table>
						<tr>
							...
							<th>创建时间：</th>
							<td><input type='text' id='addtime' class='easyui-textbox' size='16' data-options=''/></td>
							<th>ASN编号：</th>
							<td><input type='text' id='asnno' class='easyui-textbox' size='16' data-options=''/></td>
							<th>关闭时间：</th>
							<td><input type='text' id='edittime' class='easyui-textbox' size='16' data-options=''/></td>
							<td>
	...
	<div id='ezuiDialog' style='padding: 10px;'>
		<form id='ezuiForm' method='post'>
			<input type='hidden' id='viewAsnDetailId' name='viewAsnDetailId'/>
			<table>
				<tr>
					<th>待输入0</th>
