...
$(function() {
	...
	ezuiDatagrid = $('#ezuiDatagrid').datagrid({
		url : '<c:url value="/rptOrderPackingshipController.do?showDatagrid"/>',
		method:'POST',
		toolbar : '#toolbar',
		title: '',
		pageSize : 50,
		...
		columns : [[
            ...
            {field: 'soreference1',		title: '订单号 ',	width: 110 },
            ...
            {field: 'qty',		title: '装箱数 ',	width: 60 },
			{field: 'addtime',		title: '订单创建时间 ',	width: 100 },
			{field: 'lastshipmenttime',		title: '订单发运时间',	width: 100 },
			{field: 'soreference3',		title: '配载单号 ',	width: 150 },
		...
		onRowContextMenu : function(event, rowIndex, rowData) {
			...
			ajaxBtn($('#menuId').val(), '<c:url value="/rptOrderPackingshipController.do?getBtn"/>', ezuiMenu);
			$(this).datagrid('unselectAll');
		}
	});
...
var del = function(){
	...
	if(row){
		$.messager.confirm('<spring:message code="common.message.confirm"/>', '<spring:message code="common.message.confirm.delete"/>', function(confirm) {
			if(confirm){
				$.ajax({
					url : 'rptOrderPackingshipController.do?delete',
					data : {id : row.id},
					type : 'POST',
					dataType : 'JSON',
...
var commit = function(){
	var url = '';
	if (processType == 'edit') {
		url = '<c:url value="/rptOrderPackingshipController.do?edit"/>';
	}else{
		url = '<c:url value="/rptOrderPackingshipController.do?add"/>';
	}
...
var doExport = function(){
	if(navigator.cookieEnabled){
		...
		//--导出Excel
		var formId = ajaxDownloadFile(sy.bp()+"/rptOrderPackingshipController.do?exportOrderShipDataToExcel", param);
...
<body>
	...
	<div id='ezuiDialog' style='padding: 10px;'>
		<form id='ezuiForm' method='post'>
			<input type='hidden' id='rptOrderPackingshipId' name='rptOrderPackingshipId'/>
			<table>
				<tr>
					<th>待输入0</th>
				...
				<tr>
					<th>待输入10</th>
					<td><input type='text' name='soreference3' class='easyui-textbox' size='16' data-options='required:true'/></td>
				</tr>
				<tr>
					<th>待输入11</th>
