...
public class ImportOrderDataService {
	...
	private List<OrderHeaderForNormalVO> listToBean(List<ImportOrderData> orderList, StringBuilder resultMsg) {
		...
		Integer count = 1;
		String customerId = "", orderCode = "", requiredDeliveryTime = "", consigneeName = "", address = "", tel = "";
		...
		for(ImportOrderData dataArray : orderList) {
			...
			try {
				if (StringUtils.isEmpty(dataArray.getOrderno())) {
					throw new Exception();
				}
			} catch (Exception e) {
			...
			orderCode = importDataVO.getOrderCode();
			//requiredDeliveryTime = dataArray.getRequiredDeliveryTime();
			consigneeName = importDataVO.getConsigneename();
