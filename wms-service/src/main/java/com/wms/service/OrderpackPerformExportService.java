 ...
 * @Date 
 */
@Service("OrderpackPerformExportService")
public class OrderpackPerformExportService {
	...
	public void exportOrderPackDataToExcel(HttpServletResponse response, OrderpackPerformExportForm form) throws IOException {
		Cookie cookie = new Cookie("exportToken",form.getToken());
		cookie.setMaxAge(60);	
		response.addCookie(cookie);
		response.setContentType(ContentTypeEnum.csv.getContentType());
		
		RptOrderPackingPerformForm rptOrderPackingPerformForm = new RptOrderPackingPerformForm();
		
		rptOrderPackingPerformForm.setCustomerid(form.getCustomerid());
		rptOrderPackingPerformForm.setUserId(form.getUserId());
		rptOrderPackingPerformForm.setSdate(form.getSdate());
		rptOrderPackingPerformForm.setSdatetext(form.getSdatetext());
		...
		try {  
			...
			BeanUtils.copyProperties(rptOrderPackingPerformForm, query);
	        ...
	        // excel的sheetName  
	        String sheetName = "订单拣货绩效统计";   
	        // excel要导出的数据  
	        List<RptOrderPackingPerform> orderPerform = rptOrderPackingPerformMybatisDao.queryByList(mybatisCriteria); 
	        // 导出  
	        if (orderPerform == null || orderPerform.size() == 0) {  
	        	//System.out.println("题库为空");  
	        }else {  
	            //将list集合转化为excle  
	            ExcelUtil.listToExcel(orderPerform, fieldMap, sheetName, response);  
