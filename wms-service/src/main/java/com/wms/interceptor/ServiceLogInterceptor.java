...
public class ServiceLogInterceptor implements MethodInterceptor{
	...
	public Object invoke(MethodInvocation invocation) throws Throwable {
		...
		try {
			logger.log(LOG_LEVEL_METHOD, info.append("==========================").append("Start，執行ID：Service-").append(uuid).append("=============================").toString());info.setLength(0);
			logger.log(LOG_LEVEL_METHOD, info.append("執行:").append(invocation.getMethod()).toString());info.setLength(0);
			Object[] args = invocation.getArguments();
			info.append("\n");
			if (args.length > 0) {
				for (int i = 0; i < args.length; i++) {
					info.append("\t\t").append("執行參數:[" + i + "] = ");
					if (nonVO(args[i])) {
						info.append(args[i]);
					} else {
				...
				if (nonVO(result)) {
					if (result instanceof List) {
						...
						if (resultList != null && resultList.size() > 10) {
							...
							info.append("\t\t\t總資料長度為").append(resultList.size()).append("，只顯示前5筆資料");
						} else {
			...
			logger.log(LOG_LEVEL_METHOD, info.append("執行時間：").append(System.currentTimeMillis() - startTime).append(" ms.").toString());info.setLength(0);
			logger.log(LOG_LEVEL_METHOD, info.append("==========================").append("Finish，執行ID：Service-").append(uuid).append("=============================").toString());info.setLength(0);
		}
		return result;
	}
